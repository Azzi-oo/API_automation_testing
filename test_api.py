...
def test_put_object(obj_id):
    ...
    "data": {
        "year": 2020,
        "price": 1849.99,
        "CPU model": "Intel Core i1000",
        "Hard disk size": "1 VD"
    }
    }
    response = requests.put(
        f'https://api.restful-api.dev/objects/{obj_id}', json=payload
        ).json()
    assert response['name'] == payload['name']

def test_delete_object(obj_id):
    response = requests.delete(f'https://api.restful-api.dev/objects/{obj_id}').json()
